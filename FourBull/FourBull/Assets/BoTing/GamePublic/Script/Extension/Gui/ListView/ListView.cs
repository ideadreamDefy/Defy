...
public class ListView : MonoBehaviour
{
    ...
    void CreateView()
    {
        int cachedamout = cachedList.Count;
        mChild.Clear();
        for (int i = 0; i < PreCreatedAmout; i++)
        {
            if (i >= cachedamout)
                mChild.Add(CreateItem());
            else
                mChild.Add(cachedList[i].transform);
        }
        RectTransform ts = (RectTransform)ListItem.transform;
        cell_x = ts.sizeDelta.x;
        cell_y = ts.sizeDelta.y;
    }


    List<GameObject> cachedList = new List<GameObject>();
    protected virtual Transform CreateItem()
    {
        GameObject itemobj = Instantiate(ListItem);
        cachedList.Add(itemobj);
        RectTransform ts = (RectTransform)itemobj.transform;
        ts.SetParent(this.transform);
        ts.pivot = Vector2.up;
        ts.anchorMax = Vector2.up;
        ts.anchorMin = Vector2.up;
    ...
    public void UpdateItem(int realIndex)
    {
        for (int i = 0, imax = mChild.Count; i < imax; i++)
        {
            Transform temp = mChild[i];
            if (getRealIndex(temp.localPosition) == realIndex)
            {
                OnUpdateItem(temp,i,realIndex);
            }
        }
    ...
    protected virtual void OnUpdateItem(
        Transform item, int index, int realIndex)
    {
